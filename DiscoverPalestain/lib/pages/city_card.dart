...
class CityCard extends StatelessWidget {
  ...
  void editCity(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  // دالة لعرض الأماكن في المدينة
  void showPlaces(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PlaceListScreen(cityId: city.id), // تمرير city.id إلى PlaceListScreen
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    String imageUrl = city.imageUrl?.startsWith('http') == true
        ? city.imageUrl!
        : 'http://192.168.1.141/FinalProject_Graduaction/Resturants/images/${city.imageUrl?.startsWith('/') == true ? city.imageUrl!.substring(1) : city.imageUrl}';
            ...
            child: Column(
              ...
              children: [
                ...
                if (city.population != null)
                  Row(
                    children: [
                      Icon(Icons.groups, color: Colors.teal, size: 18),
                      SizedBox(width: 4),
                      Text(
                ...
                // زر عرض الأماكن وأزرار التعديل والحذف في نفس الـ Row
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween, // توزيع العناصر بين اليسار واليمين
                  children: [
                   ElevatedButton(
  onPressed: () => showPlaces(context),  // عرض الأماكن عند الضغط
  child: Text(
    "Show Places",
    style: TextStyle(color: Colors.white),  // تغيير لون النص إلى الأبيض
  ),
  style: ElevatedButton.styleFrom(
    backgroundColor: Colors.teal,  // تغيير لون الخلفية إلى الأخضر الداكن
  ),
)
,
                    Row(
                      children: [
                        IconButton(
                          onPressed: () => editCity(context),
                          icon: Icon(Icons.edit, color: Colors.orange),
                          tooltip: "Edit City",
                        ),
                        IconButton(
                          onPressed: () => deleteCity(context),
