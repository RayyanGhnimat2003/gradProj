...
class AddPlaceScreen extends StatefulWidget {
  final int cityId;  // نضيف الـ cityId للربط مع المدينة

  AddPlaceScreen({required this.cityId});
...
class _AddPlaceScreenState extends State<AddPlaceScreen> {
  ...
  Future<void> uploadPlace() async {
    ...
    final uri = Uri.parse('http://192.168.1.141/FinalProject_Graduaction/City/add_place.php');
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Place added successfully!')));
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          labelText: label,
          prefixIcon: Icon(icon, color: Colors.teal), // تغيير لون الأيقونة فقط إلى Teal
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Add Place'), backgroundColor: Colors.teal),
      body: Padding(
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              buildTextField('Place Name', Icons.location_city, _nameController, required: true),
              ...
              TextButton.icon(
                icon: Icon(Icons.image, color: Colors.teal),  // تغيير لون الأيقونة فقط
                label: Text('Choose Image', style: TextStyle(color: Colors.teal)),  // تغيير لون النص
                onPressed: pickImage,
              ),
              SizedBox(height: 16),
              ElevatedButton.icon(
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate()) uploadPlace();
                },
